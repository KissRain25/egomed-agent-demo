...
This script uses SAM3 official image processor:
  SAM3 base checkpoint only

Each prompt-frame row is evaluated independently:
  image_path + row["prompt"] -> SAM3 -> selected pred mask
...
Important:
  ...
  from the CSV by default:
      SAM3_TEXT_SOURCE = "prompt"

Run:
  conda activate sam2
  cd $EGOMED_ROOT
  python eval_sam3_image_text_prompt_egomed5_save_outputs.py
"""
...
RUN_ROOT = (
    Path(f"{REPO_ROOT}/runs/eval_sam3_image_text_prompt")
...
# SAM3 image processor returns masks, boxes, and scores.
# No SAM3 LoRA / detection threshold is used here.

# Select one prediction from SAM3 output without using GT.
SELECT_MASK_MODE = "top_score"

# Threshold masks. If SAM3 mask values look like logits, code uses > 0.
...
def save_config_json():
    ...
    cfg = {
        ...
        "DEVICE": DEVICE,
        "SAM3_TEXT_SOURCE": SAM3_TEXT_SOURCE,
...
# =============================================================================
# Import SAM3 official image processor
# =============================================================================
...
def normalize_sam3_text(row: pd.Series) -> str:
    if SAM3_TEXT_SOURCE == "target_class":
...
def get_prompt_prediction(raw_result: Any, prompt_index: int = 0) -> Dict[str, Any]:
    ...
    raise ValueError(f"Unsupported SAM3 predict result format: {type(raw_result)}")
...
class SAM3ImageTextPredictor:
    ...
    def load(self):
        ...
        os.chdir(SAM3_REPO_ROOT)

        self.model = build_sam3_image_model(
            checkpoint_path=str(SAM3_CHECKPOINT),
        )

        # Keep model weights in float32.
        # SAM3 processor/image tensors may use bf16 internally;
        # use autocast during forward instead of manually converting model dtype.
    ...
    def predict(self, image_path: str, text_prompt: str):
        ...
        # SAM3 image processor can create bf16 image tensors on CUDA.
        # Keep model fp32, and use autocast for both image encoder and text grounding.
        # This avoids both:
        #   BFloat16 input vs Float weight
        #   Float input vs BFloat16 weight
        if DEVICE == "cuda":
            with torch.autocast("cuda", dtype=torch.bfloat16):
                state = self.processor.set_image(image_pil)
                raw_result = self.processor.set_text_prompt(
                    state=state,
        ...
        if n_masks == 0:
            ...
            elif SELECT_MASK_MODE == "largest_mask":
                areas = np.array([mask_to_bool(m).sum() for m in masks], dtype=np.float64)
            ...
            selected_box = (
                boxes_xyxy[selected_idx]
                if selected_idx < len(boxes_xyxy)
...
def main():
    ...
    print("=" * 120)
    print("SAM3 Image Text-Prompt Evaluation on EgoMed5 Test Online Schedule")
    print("=" * 120)
    print(f"Prompt CSV:          {PROMPT_CSV}")
    print(f"SAM3 repo root:   {SAM3_REPO_ROOT}")
    print(f"Device:              {DEVICE}")
    print(f"Run root:            {RUN_ROOT}")
    print(f"Text source:         {SAM3_TEXT_SOURCE}")
    ...
    for _, row in tqdm(df.iterrows(), total=len(df), desc="SAM3 image eval"):
        ...
        sam3_text_prompt = normalize_sam3_text(row)
        ...
        try:
            ...
            out = predictor.predict(image_path=image_path, text_prompt=sam3_text_prompt)
