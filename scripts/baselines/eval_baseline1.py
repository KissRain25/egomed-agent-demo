...
"""
Init-Only Prop. (paper Table II / module ablation): only the Propagation Agent ---
YOLO26 detector initializes the target once, then original SAM2 propagates; no Consistency Evaluation.

Mode:
  schedule-aware one-time detector initialization + SAM2 video propagation.

Fair schedule rule:
  A target class is NOT allowed before its prompt_activate_frame.
  After activation, the first YOLO detection is used once to initialize SAM2.
  After initialization, SAM2 propagates the target mask to subsequent frames.
  No per-frame SAM2 image prompt.
  No IoU correction.
  No reinitialization.
  No retracking.

This baseline tests:
  "Has propagation, but no detection-guided correction."

Run:
  conda activate sam2
  cd $EGOMED_ROOT
  python eval_baseline1.py
"""
...
DATASET_CONFIGS = {
    ...
    "PolypGen2021_MultiCenterData_v3": {
        "yolo_weight": YOLO_RUN_ROOT / "PolypGen2021_MultiCenterData_v3" / "weights" / "best.pt",
    },
}

# If your PolypGen folder is actually named with imgsz1024, the script will auto-fallback below.
POLYPGEN_FALLBACK_WEIGHT = (
    YOLO_RUN_ROOT / "PolypGen2021_MultiCenterData_v3_yolo26m_imgsz1024" / "weights" / "best.pt"
)
...
RUN_ROOT = (
    Path(f"{REPO_ROOT}/runs/eval_baseline1_detector_init_sam2_video_schedule")
    / "egomed5_yolo26m_original_sam2_video_init_once_schedule"
)
...
def build_one_time_init_plan(detections_by_frame, case_schedule, focus_class_ids):
    """
    For each class:
      find the first frame >= prompt_activate_frame where YOLO detects this class.
    This detection initializes SAM2 once.
    """
    init_plan = {}

    for class_id in sorted(focus_class_ids):
        if class_id not in case_schedule:
            continue

        activate_frame = int(case_schedule[class_id])
        start = max(0, activate_frame)

        for frame_idx in range(start, len(detections_by_frame)):
...
# ============================================================
# SAM2 video predictor utilities
# ============================================================

def init_sam2_video_predictor():
...
def overlay_prediction(
    ...
    # Blue: YOLO boxes. These are shown for visualization only.
    # Baseline1 does NOT use them after one-time initialization.
    ...
    if DRAW_TRACK_BOXES_ON_OVERLAY and track_boxes:
        for class_id, box in track_boxes.items():
            ...
            cv2.putText(
                vis,
                f"SAM2-prop {id_to_name.get(class_id, class_id)}",
    ...
    # Cyan: one-time initialization bbox on its init frame.
    if init_plan is not None and frame_idx is not None:
        for class_id, item in init_plan.items():
            ...
            cv2.putText(
                vis,
                f"INIT {id_to_name.get(class_id, class_id)}",
                (x1, min(vis.shape[0] - 2, y2 + 30)),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.45,
                (255, 255, 0),
                1,
...
def evaluate_case_dice_from_saved_masks(
    ...
    for class_id, class_name in id_to_name.items():
        ...
        det_gt_ious = stats.get("init_det_gt_iou", [])
        ...
        rows.append({
            ...
            "init_det_conf": stats.get("init_det_conf", np.nan),
            "init_det_gt_iou": float(np.mean(det_gt_ious)) if det_gt_ious else np.nan,
        })
...
def evaluate_case(
    ...
    # Count detection frames before/after schedule activation.
    ...
    # 2. Build one-time initialization plan.
    init_plan = build_one_time_init_plan(
    ...
    if len(init_plan) == 0:
        print(f"[Warning] No active YOLO initialization in {dataset_name} case {case_id}. Saving empty masks.")
    ...
    first_start_idx = min(item["frame_idx"] for item in init_plan.values())

    print(
        f"[{dataset_name} case {case_id}] Baseline1 initialize once. "
        f"First start frame: {first_start_idx}. "
        f"Init classes: "
        f"{[(id_to_name[c], item['frame_idx']) for c, item in sorted(init_plan.items(), key=lambda x: x[1]['frame_idx'])]}"
    )

    # Save empty predictions before first initialization frame.
    ...
    # 3. Initialize SAM2 video state once.
    print(f"[{dataset_name} case {case_id}] Loading SAM2 frames once...")
    ...
    # Add each class once at its first active detection frame.
    for class_id, item in sorted(init_plan.items(), key=lambda x: x[1]["frame_idx"]):
        init_frame_idx = int(item["frame_idx"])
        det = item["det"]
        obj_id = next_obj_id
        next_obj_id += 1
        ...
        tracking_stats[class_id]["registered_frame"] = init_frame_idx
        tracking_stats[class_id]["num_registrations"] = 1
        tracking_stats[class_id]["init_det_conf"] = float(det["conf"])

        gt_label = cv2.imread(str(gt_label_files[init_frame_idx]), cv2.IMREAD_GRAYSCALE)
        if gt_label is not None:
            gray_value = int(class_id_to_gray[class_id])
            gt_mask = gt_label == gray_value
            gt_box = mask_to_bbox(gt_mask)
            tracking_stats[class_id]["init_det_gt_iou"].append(bbox_iou(det["box"], gt_box))

    # 4. Propagate once. No correction. No reinitialization.
    saved_frames = set()

    for out_frame_idx, out_obj_ids, out_mask_logits in propagate_video(
        predictor=predictor,
        inference_state=inference_state,
        start_frame_idx=first_start_idx,
    ):
        out_frame_idx = int(out_frame_idx)
        if out_frame_idx >= num_frames:
            break

        masks_by_class = sam2_masks_from_output(
            out_obj_ids=out_obj_ids,
            out_mask_logits=out_mask_logits,
            obj_id_to_class_id=obj_id_to_class_id,
        )

        # Do not allow a class to appear before its own initialization frame.
        filtered_masks_by_class = {}
        for class_id, mask in masks_by_class.items():
            if class_id not in init_plan:
                continue
            if out_frame_idx < int(init_plan[class_id]["frame_idx"]):
                continue
            filtered_masks_by_class[class_id] = mask

        track_boxes = {
            class_id: mask_to_bbox(mask)
            for class_id, mask in filtered_masks_by_class.items()
        }

        pred_label = compose_pred_label(
            mask_by_class=filtered_masks_by_class,
            class_id_to_gray=class_id_to_gray,
            frame_shape=(frame_h, frame_w),
        )

        save_pred_label(pred_label, case_pred_dir / f"{image_files[out_frame_idx].stem}.png")
        saved_frames.add(out_frame_idx)
        ...
        if SAVE_OVERLAY:
            ...
            active = active_class_ids_at_frame(case_schedule, out_frame_idx)
            active_dets = filter_detections_by_active(detections_by_frame[out_frame_idx], active)
            overlay_prediction(
                image_rgb=image_rgb,
    ...
    # Fill unsaved frames with empty masks.
    for frame_idx, image_path in enumerate(image_files):
...
def resolve_yolo_weight(dataset_name):
    yolo_weight = DATASET_CONFIGS[dataset_name]["yolo_weight"]

    if dataset_name == "PolypGen2021_MultiCenterData_v3":
        if not yolo_weight.exists() and POLYPGEN_FALLBACK_WEIGHT.exists():
            yolo_weight = POLYPGEN_FALLBACK_WEIGHT
...
def evaluate_dataset(dataset_name, predictor):
    ...
    yolo_weight = resolve_yolo_weight(dataset_name)

    dataset_run_root = RUN_ROOT / dataset_name
    pred_mask_root = dataset_run_root / "pred_mask"
    overlay_root = dataset_run_root / "overlay"
    excel_output = dataset_run_root / f"{dataset_name}_baseline1_yolo26m_original_sam2_video_init_once_schedule.xlsx"
    ...
    print("\n" + "=" * 120)
    print(f"Evaluating {dataset_name}: Baseline1 one-time YOLO initialization + SAM2 video propagation")
    print("=" * 120)
    ...
    if len(dice_df) > 0:
        ...
        class_summary = (
            ...
            .agg(
                ...
                mean_registered_frame=("registered_frame", "mean"),
                mean_init_det_conf=("init_det_conf", "mean"),
                mean_init_det_gt_iou=("init_det_gt_iou", "mean"),
            )
            .reset_index()
    ...
    if len(class_summary) > 0:
        ...
        display_cols = [
            ...
            "mean_registered_frame",
            "mean_init_det_conf",
            "mean_init_det_gt_iou",
        ]
...
def main():
    ...
    print("=" * 120)
    print("Baseline1: YOLO26-det initialized original SAM2 Video Predictor")
    print("Mode: schedule-aware one-time YOLO bbox initialization + SAM2 propagation")
    print("No per-frame image prompt. No correction. No retrack.")
    print("=" * 120)
    ...
    print("\nLoading original SAM2 video predictor...")
    predictor = init_sam2_video_predictor()
