...
INPUT_CSV = Path(f"{REPO_ROOT}/data/text_prompt_eval/target_confirmation_eval_detector_generated_v3_sample150.csv")

OUT_DIR = Path(f"{REPO_ROOT}/runs/target_confirmation_eval/detector_generated_v3_sample150_deepseek")
...
def compact_candidate_objects(candidate_objects):
    ...
    Spatial relations must be inferred from bbox coordinates.
    """
...
- For "object on the left", "left side of the image", or "visually left", choose the candidate with the smallest bbox center x = (x1 + x2) / 2.
- For "object on the right", "right side of the image", or "visually right", choose the candidate with the largest bbox center x = (x1 + x2) / 2.
...
Important distinction between anatomical left/right and image-space left/right:
- If the instruction explicitly names an anatomical category, such as "left kidney", "right kidney", "left lung", "right lung", "Left Ventricle", or "Left Atrium", treat left/right as part of the category name and match the candidate category.
- If the instruction says "object on the left", "object on the right", "left side of the image", or similar visual/spatial wording, treat left/right as image-space direction and determine it only from bbox coordinates.
- Do not use anatomical knowledge or the word "left/right" inside a category name to answer image-space spatial instructions.
- Always make target_id and target_category consistent with the coordinate calculation.

Coordinate convention for spatial instructions:
- The bbox format is [x1, y1, x2, y2].
- The image coordinate origin is the top-left corner.
- x increases from left to right.
- y increases from top to bottom.
- For "object on the left", choose the candidate with the smallest bbox center x = (x1 + x2) / 2.
- For "object on the right", choose the candidate with the largest bbox center x = (x1 + x2) / 2.
- For "upper object", choose the candidate with the smallest bbox center y = (y1 + y2) / 2.
- For "lower object", choose the candidate with the largest bbox center y = (y1 + y2) / 2.
- For "larger bounding box", choose the candidate with the largest area = (x2 - x1) * (y2 - y1).
- For "smaller bounding box", choose the candidate with the smallest area = (x2 - x1) * (y2 - y1).
- Do not use anatomical left/right or category names such as "left kidney", "right lung", "Left Ventricle", or "Left Atrium" to infer image-space left/right.
- Always make target_id and target_category consistent with the coordinate calculation.
...
3. If the instruction uses image-space spatial information, follow the coordinate convention above strictly. If the instruction names an anatomical category such as "left kidney" or "right lung", match the category instead of using image-space coordinates.
...
Coordinate convention for spatial clarification:
- The bbox format is [x1, y1, x2, y2].
- The image coordinate origin is the top-left corner.
- x increases from left to right.
- y increases from top to bottom.
- For left/right, compare bbox center x.
- For upper/lower, compare bbox center y.
- For larger/smaller bounding box, compare bbox area.
- Do not use anatomical left/right or category names to infer image-space left/right.

Coordinate convention for image-space spatial clarification:
- The bbox format is [x1, y1, x2, y2].
- The image coordinate origin is the top-left corner.
- x increases from left to right in the image.
- y increases from top to bottom in the image.
- For image-space left/right, compare bbox center x.
- For upper/lower, compare bbox center y.
- For larger/smaller bounding box, compare bbox area.
- If the clarification names an anatomical category such as "left kidney" or "right lung", match the category.
- If the clarification says "on the left/right side of the image", use bbox coordinates.
...
5. If the clarification mentions image-space spatial information, infer the target directly from the candidate bounding boxes using the coordinate convention above. If it names an anatomical category, match the category.
