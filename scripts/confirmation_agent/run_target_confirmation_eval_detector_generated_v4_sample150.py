...
INPUT_CSV = Path(f"{REPO_ROOT}/data/text_prompt_eval/target_confirmation_eval_detector_generated_v4_sample150.csv")

OUT_DIR = Path(f"{REPO_ROOT}/runs/target_confirmation_eval/detector_generated_v4_sample150_deepseek")
...
- "unique": the instruction explicitly specifies a unique target category among the candidates.
- "ambiguous": the instruction does not explicitly specify a unique target category and should be clarified before segmentation.
...
2. Semantic vague instructions are ambiguous. For example, "Segment the kidney", "Segment the lung", "Show me the cardiac chamber", or "Highlight the cardiac structure" may refer to multiple candidate targets.
3. Image-space spatial references are also treated as ambiguous in this evaluation because they do not explicitly name the target category. For example, "Segment the object on the left side of the image", "Segment the lower object", or "Segment the object with the larger bounding box" should trigger clarification.
4. Do not invent candidate targets. The target_id must be one of the candidate ids.
5. If the grounding state is "ambiguous", target_id and target_category should be empty strings because the final target should be confirmed after user clarification.
6. The score measures the reliability of grounding between the user instruction and the candidate targets.
...
Scoring rule:
- Use 0.80 to 1.00 when the instruction explicitly names exactly one candidate category.
- Use below 0.50 when the instruction is semantically vague or uses only image-space spatial reference and therefore requires clarification.
