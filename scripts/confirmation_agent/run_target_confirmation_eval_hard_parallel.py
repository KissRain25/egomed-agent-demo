...
INPUT_CSV = Path(f"{REPO_ROOT}/data/text_prompt_eval/target_confirmation_eval_hard.csv")

OUT_DIR = Path(f"{REPO_ROOT}/runs/target_confirmation_eval/hard_v4flash")
...
def compact_candidate_objects(candidate_objects):
    """
    Keep candidate fields from the Detection Agent:
    id, category, bounding box, and confidence.
    """
    ...
    for obj in objs:
        compact.append({
            ...
            "bbox": obj.get("bbox", []),
            "confidence": float(obj.get("confidence", 0.0)) if str(obj.get("confidence", "")).strip() else 0.0,
        })
...
2. A set of candidate medical targets produced by the Detection Agent.
...
- "unique": the instruction provides enough semantic or image-space spatial information to reliably identify exactly one candidate target.
- "ambiguous": the instruction does not provide enough information to reliably identify exactly one candidate target and should be clarified before segmentation.

Coordinate convention for image-space spatial instructions:
- The bbox format is [x1, y1, x2, y2].
- The image coordinate origin is the top-left corner.
- x increases from left to right in the image.
- y increases from top to bottom in the image.
- For "object on the left side of the image", choose the candidate with the smallest bbox center x = (x1 + x2) / 2.
- For "object on the right side of the image", choose the candidate with the largest bbox center x = (x1 + x2) / 2.
- For "upper object", choose the candidate with the smallest bbox center y = (y1 + y2) / 2.
- For "lower object", choose the candidate with the largest bbox center y = (y1 + y2) / 2.
- For "larger bounding box", choose the candidate with the largest area = (x2 - x1) * (y2 - y1).
- For "smaller bounding box", choose the candidate with the smallest area = (x2 - x1) * (y2 - y1).

Object-relative spatial instructions (relative to a named reference structure, e.g. an organ):
- First locate the reference structure among the candidates by its category name (for example "the liver"), then reason using bbox centers.
- "immediately to the left of the reference": among candidates whose center x is smaller than the reference center x, choose the one with the LARGEST center x (the nearest one on the left). This is unique.
- "immediately to the right of the reference": among candidates whose center x is larger than the reference center x, choose the SMALLEST center x (nearest on the right). Unique.
- "immediately above or below the reference": the nearest candidate above or below the reference by center y. Unique.
- "on the left of the reference" WITHOUT the word "immediately": if exactly one candidate is to the left of the reference it is unique; if two or more candidates are to the left, it is ambiguous and must be clarified. The same holds for right, above, and below.

Important rules:
1. Explicit category instructions are unique. For example, "Segment the left kidney", "Segment the right lung", "Segment the LV cavity", or "Segment the gallbladder" should be grounded by matching the candidate category.
2. Clear image-space spatial instructions are unique when the bbox relation identifies exactly one candidate, such as "Segment the lower object" or "Segment the object with the larger bounding box".
3. Semantic vague instructions are ambiguous. For example, "Segment the kidney", "Segment the lung", "Show me the cardiac chamber", or "Highlight the cardiac structure" may refer to multiple candidate targets.
4. Underspecified spatial references are ambiguous. For example, "Segment one of the nearby objects" does not identify a unique target and should trigger clarification.
5. If the instruction uses image-space left/right, use bbox coordinates. If the instruction names an anatomical category such as "left kidney" or "right lung", match the category instead.
6. Do not invent candidate targets. The target_id must be one of the candidate ids.
7. If the grounding state is "ambiguous", target_id and target_category should be empty strings because the final target should be confirmed after user clarification.
8. The score measures the reliability of grounding between the user instruction and the candidate targets.
...
- Use below 0.50 when the instruction is semantically vague or spatially underspecified and therefore requires clarification.
...
2. The candidate medical targets produced by the Detection Agent.
...
5. If the clarification mentions image-space spatial information, infer the target directly from the candidate bounding boxes.
6. If the original instruction used spatial reference but the clarification names a target category, use the clarified category as the final user-intended target.
...
def main():
    ...
    todo = [row for _, row in df.iterrows() if str(row["sample_id"]) not in done_keys]

    def _process_one(row):
        candidate_objects = compact_candidate_objects(row["candidate_objects"])
        instruction = str(row["instruction"])
        initial = call_initial_confirmation_agent(client, model_name, instruction, candidate_objects)
        ...
        if clarification_response.strip():
            clarified = call_clarification_confirmation_agent(client, model_name, instruction, candidate_objects, clarification_response)
        else:
            clarified = {"clarification_raw": "", "clarification_api_error": "", "confirmed_target_id": "", "confirmed_target_category": "", "clarification_reason": ""}
        out_row = row.to_dict()
        out_row.update({
            "candidate_objects_compact": json.dumps(candidate_objects, ensure_ascii=False),
            "grounding_state": initial["grounding_state"], "target_id": initial["target_id"],
            "target_category": initial["target_category"], "score": initial["score"],
            "initial_reason": initial["initial_reason"], "initial_raw": initial["initial_raw"],
    ...
    with ThreadPoolExecutor(max_workers=26) as ex:
        for out_row in tqdm(ex.map(_process_one, todo), total=len(todo), desc="DeepSeek (parallel x26)"):
            rows.append(out_row)
    pd.DataFrame(rows).to_csv(PRED_CSV, index=False)

    pred_df = pd.DataFrame(rows)
    pred_df["sample_id"] = pred_df["sample_id"].astype(str)
    pred_df = pred_df.sort_values("sample_id")
