...
def train_one_dataset(dataset_name):
    ...
    print("=" * 100)
    print("Training YOLO26m detector")
    ...
    def on_fit_epoch_end(trainer):
        ...
        current_top3 = sorted(
            [(e, f"{s:.4f}") for s, e, _ in top3_heap],
            reverse=True,
        )
        print(
            f"[Top3] Epoch {epoch}: mAP50-95={map_score:.4f} | "
            f"Current top3: {current_top3}"
        )
    ...
    results = model.train(
        ...
        # medical image detection augmentation settings
        fliplr=0.0,
        flipud=0.0,
        degrees=0.0,
        translate=0.05,
